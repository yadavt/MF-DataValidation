...
zip -r9 json-validation.zip ../package/
zip -g json-validation.zip json-validation.py

#Create the lambda fucntion
aws lambda create-function --function-name $functionName \
--zip-file fileb://json-validation.zip --handler json-validation.lambda_handler --runtime python3.7 \
